...
function getFailureFromError(item: ImageImportItem, error: unknown) {
  ...
  return buildFailure(item, 'write', error instanceof Error ? error.message : '追加に失敗しました。');
}
...
export async function importImageBatch({
  ...
  const importedImages: ImageRecord[] = [];
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    try {
      ...
      if (item.sourceKind === 'picker-handle') {
        ...
        const removed = item.fileHandle ? await fileSystem.removeFile(item.fileHandle) : false;
        if (!removed) {
          throw new ImportStageError(
            'source delete',
            'delete-source',
      ...
      if (importedImageId) {
        ...
        const importedImageListIndex = importedImages.findIndex((image) => image.id === importedImageId);
        if (importedImageListIndex >= 0) importedImages.splice(importedImageListIndex, 1);
